...
training data a model can actually learn from.
    ...
    python -m ml.pipelines.backfill --cities islamabad     # just these, others untouched
...
def run_backfill(
    ...
    write_features(combined, fs=fs)

    print("\n  Feature store now holds:")
    print(table_summary(fs=fs).to_string(index=False))
    print()
    return combined
