...
Two outputs:
  ...
  2. Per-city drivers for the *current* forecast, written to the Hopsworks
     `forecast_drivers` Feature Group, so the dashboard can show "why".
...
from ml.storage.hopsworks_store import get_feature_store, write_drivers  # noqa: E402
...
def run_explain() -> pd.DataFrame:
    ...
    data = add_targets(read_features(stride=EXPLAIN_STRIDE, fs=fs))
    ...
    # Explain the fitted estimator on transformed inputs.
    pre = model[:-1]
    estimator = model[-1]
    ...
    write_drivers(drivers, fs=fs)
    print(f"\n  Wrote {len(drivers)} drivers to Hopsworks.\n")
    return drivers
